...
const Dashboard = {
  render() {
    ...
    if (nel) { nel.textContent = Data.fmt(net); nel.style.color = net >= 0 ? '#0F6E56' : '#A32D2D'; }
  ...
  _renderRecap(jj) {
    if (!jj || jj.length === 0) {
      ...
      ['rj-s','rj-b','rj-c','rj-esp','rj-dep','rj-solde'].forEach(id => this._set(id, '0 FCFA'));
    ...
    if (sel) {
      ...
      sel.style.color = solde >= 0 ? '#0F6E56' : '#A32D2D';
    }
